...
def create_or_update_component(resource, device_type_obj, item, allowed_fields, nb=None, created_map=None):
    ...
    device_type_obj: the device type Record
    ...
    # Always include device_type for template creation
    payload["device_type"] = device_type_obj.id
    ...
    # front-ports.rear_port -> prefer IDs created earlier in this run, then lookup by name in dcim.rear_port_templates filtered by device_type
    if "rear_port" in payload and payload["rear_port"] is not None:
        try:
            ...
            # 2) if not resolved yet, try NetBox API filtered by device_type
            if not isinstance(payload["rear_port"], int):
                if nb_api:
                    ...
                    if not rear_candidates:
                        # fallback: fetch all rear ports for device_type and match locally
                        all_rear_ports = list(nb_api.dcim.rear_port_templates.filter(device_type=device_type_obj.id))
                    ...
                    if rear:
                        ...
                        print(f"    WARNING: rear_port '{name_val}' not found for device_type {device_type_obj.id}")
    ...
    # power-outlets.power_port -> lookup in dcim.power_port_templates filtered by device_type
            ...
            if not isinstance(payload.get("power_port"), int):
                if nb_api:
                    ...
                    if pwr:
                        ...
                        # not found for this device type -> remove to avoid creating invalid reference
    ...
    # Find existing template by name + device_type
    existing = None
    try:
        if "name" in payload and payload["name"] is not None:
            # Query NetBox for templates by name (could return from other device types too)
            candidates = list(resource.filter(name=payload["name"]))
            
            # Keep only ones that belong to THIS device type
            candidates = [
                c for c in candidates
                if getattr(c, "device_type", None) and getattr(c.device_type, "id", None) == device_type_obj.id
            ]
    ...
    if existing:
        try:
            if hasattr(existing, 'device_type') and existing.device_type.id == device_type_obj.id:
                # Build payload without device_type for comparison
                update_payload = {k: v for k, v in payload.items() if k != "device_type"}
                ...
                print(f"    DEBUG: Existing template belongs to different device type, creating new one")
                existing = None
        except Exception as e:
...
def write_error_log(filepath, relative_path, message, exc=None, base_dir=None):
    """Write error details to a per-device-type log file under device_type_logs/"""
    if base_dir is None:
        base_dir = os.path.dirname(os.path.abspath(__file__))
    logs_dir = os.path.join(base_dir, "device_type_logs")
    try:
...
def process_components(netbox, device_type_obj, doc, filepath=None, base_dir=None, relative_path=None):
    ...
    for yaml_key in ordered_keys:
        ...
        if resource is None:
            ...
            print(f"WARNING: {msg}")
            # log the missing resource as a warning for this device-type
            write_error_log(filepath, relative_path, msg, base_dir=base_dir)
            continue

        for item in items if isinstance(items, list) else [items]:
            status, info = create_or_update_component(resource, device_type_obj, item, allowed, nb=netbox, created_map=created_map)
            if status == "created":
...
def process_file(netbox, filepath, tracker=None):
    ...
    relative_path = None
    device_types_dir = os.path.join(base_dir, "devicetype-library-master", "device-types")
    try:
        if os.path.commonpath([os.path.abspath(filepath), os.path.abspath(device_types_dir)]) == os.path.abspath(device_types_dir):
            relative_path = os.path.relpath(filepath, device_types_dir)
    except Exception:
    ...
    try:
        ...
        for doc in docs:
            ...
            existing = find_existing_device_type(netbox, payload)
            ...
            if existing:
                try:
                    ...
                    device_type_obj = existing
                    netbox_id = existing.id
                    print(f"Updated DeviceType: {payload.get('model') or payload.get('slug')} (id={existing.id})")
                except Exception as e:
                ...
                try:
                    created = netbox.dcim.device_types.create(payload)
                    device_type_obj = created
                    netbox_id = created.id
                    
                    # Add the 'Valid' tag to newly created device types
                    if valid_tag_id:
                        try:
                            ...
                            if valid_tag_id not in tag_ids:
                                ...
                                print(f"  Added 'Valid' tag to device type")
                        except Exception as e:
                            print(f"  WARNING: Failed to add 'Valid' tag: {e}")
                    
                    print(f"Created DeviceType: {payload.get('model') or payload.get('slug')} (id={created.id})")
                except Exception as e:
            ...
            try:
                process_components(netbox, device_type_obj, doc, filepath=filepath, base_dir=base_dir, relative_path=relative_path)
...
def main():
    ...
    tracker = DeviceTypeTracker(base_dir)
    print("Scanning device-types directory and initializing tracker...")
    if not tracker.scan_device_types():
        print("Failed to initialize tracker. Proceeding without tracking.")
        tracker = None
    else:
        print(f"Tracker initialized. Found {tracker.progress_data['total_files']} device type files.")
        tracker.print_summary()
    
    # Ask user if they want to process all files or just pending ones
    if tracker:
        pending_count = len(tracker.get_pending_device_types())
        if pending_count < tracker.progress_data['total_files']:
            print(f"\nFound {pending_count} pending device types out of {tracker.progress_data['total_files']} total.")
            choice = input("Process only pending files? (y/n): ").lower().strip()
            if choice == 'y':
                print("Processing only pending device types...")
                pending = tracker.get_pending_device_types()
    ...
    if not os.path.isdir(device_types_dir):
        print("device-types directory not found:", device_types_dir)
        return
        
    patterns = ("**/*.yml", "**/*.yaml")
    files = []
    for p in patterns:
        files.extend(glob.glob(os.path.join(device_types_dir, p), recursive=True))
    if not files:
        print("No YAML files found under:", device_types_dir)
        return
